...
vec3 EvalDiffuse(vec3 wi, vec3 wo, vec2 uv) {
  vec3 diff = GetGBufferDiffuse(uv);//反射率
  vec3 norm = GetGBufferNormalWorld(uv);//法线
...
vec3 EvalDirectionalLight(vec2 uv) {
  vec3 posW = GetGBufferPosWorld(uv);//得到世界坐标
...
void main() {
  ...
  // 曝光色调映射
    vec3 mapped = vec3(1.0) - exp(-L * 0.5);
